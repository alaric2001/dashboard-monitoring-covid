...
function MedicalPrescription() {
  ...
  const medical_prescription = router.query.medical_prescription;
  ...
  const data_by_id = data.find(
    (item) => item.id == Number(medical_prescription)
  );
  return (
    <>
      <Nav_bar />
      <div className={styles.layout}>
        ...
        <div className={styles.children}>
          ...
          <div className='mx-auto flex h-[46px] w-[1172px] items-center justify-center gap-[100px] border border-solid border-[#D7D7D7] bg-white text-black'>
            <Link
              href={`/dashboard/patient/vital_sign/${medical_prescription}`}
            >
              TTV Monitoring
            </Link>
            <Link
              href={`/dashboard/patient/medical_prescription/${medical_prescription}`}
              className='text-btn-primary'
            >
              Medical Prescription
            </Link>
            <Link href={`/dashboard/patient/order_lab/${medical_prescription}`}>
              Order Lab
            </Link>
            <Link
              href={`/dashboard/patient/order_radiology/${medical_prescription}`}
            >
              Order Radiology
            </Link>
            <Link href={`/dashboard/patient/ippn/${medical_prescription}`}>
