...
function OrderRadiology() {
  ...
  const order_radiology = router.query.order_radiology
  ...
  const data_by_id = data.find((item) => item.id == Number(order_radiology));
  return (
    <>
      <Nav_bar />
      <div className={styles.layout}>
        ...
        <div className={styles.children}>
          ...
          <div className='mx-auto flex h-[46px] w-[1172px] items-center justify-center gap-[100px] border border-solid border-[#D7D7D7] bg-white text-black'>
            <Link href={`/dashboard/patient/vital_sign/${order_radiology}`}>
              TTV Monitoring
            </Link>
            <Link
              href={`/dashboard/patient/medical_prescription/${order_radiology}`}
            >
              Medical Prescription
            </Link>
            <Link href={`/dashboard/patient/order_lab/${order_radiology}`}>
              Order Lab
            </Link>
            <Link
              href={`/dashboard/patient/order_radiology/${order_radiology}`}
              className='text-btn-primary'
            >
              Order Radiology
            </Link>
            <Link href={`/dashboard/patient/ippn/${order_radiology}`}>
